    ...
    class Overlay
    {
        ...
        public void Render(OverlayRenderer render)
        {
            ...
            if (EngineClient.IsInGame)
            {
                ...
                if(RCS)
                {
                    ...
                    if (LocalPlayer.ShotsFired > 1)
                    {
                        ...
                        EngineClient.ViewAngles = NewViewAngles;
                    }
                ...
                if (RecoilCrosshair)
                {
                    ...
                    int dx = WindowSize.Width / 90;
                    int dy = WindowSize.Height / 90;

                    x -= (int)(dx * (LocalPlayer.PunchAngle.Y));
                    y += (int)(dy * (LocalPlayer.PunchAngle.X));

                    render.Rectangle(x - 8, y, 16, 1, Color.Green, true);
                    render.Rectangle(x, y - 8, 1, 16, Color.Green, true);

                    if (!RecoilCrosshairUpdate)
                    {
                        UserCMD.Execute("crosshair 0");
                        RecoilCrosshairUpdate = true;
                    }
                }
                else
                {
            ...
            if(Menu)
            {
                ...
                Checkbox(render, 10, WindowSize.Height / 2 + 100, "Recoil Crosshair", ref RecoilCrosshair);
                Checkbox(render, 10, WindowSize.Height / 2 + 120, "RCS", ref RCS);
            }
        }
